...
Treat this file as the repo-level source of truth for agent behavior and guardrails inside `POEM`. Use the root `README.md` for the high-level human-facing project overview. For deeper runtime, UI-authoring, and automation detail, go directly to the design subjects under [`docs/`](docs/README.md): [architecture](docs/design/architecture/TDD.md) and [automation](docs/design/automation/TDD.md). `ARCHITECTURE.md`, `GUIDE.md`, and `docs/AUTOMATION.md` no longer exist — their content was subsumed into those subjects.
...
5. **One Application API, Three Layers**: Since the 2026-07-16 consolidation ([ADR-0002](docs/adr/0002-consolidation.md)), the former siblings live in this repo: `pkg/app` is the single public application API (`App[S]`/`Msg`/`Node`), `pkg/render` is the native engine layer, `pkg/web` is the server-rendered HTML component library (formerly GopherWeb). When adding or changing a public component in `pkg/render/components` or `pkg/web/components`, check the other catalog for the same concept and converge naming/shape when the need is genuinely the same; leave it alone when the difference is structural (`pkg/render`'s stateful closure-based tree vs. `pkg/web`'s per-request rendering and caller-trusted HTML fields — see the superseded-but-still-true findings in [ADR-0003](docs/adr/0003-gopherweb-parity.md)). New app-facing capability is exposed by adding a `Node` kind in `pkg/app` plus a case in **both** drivers' `render.go` — there is no partial-coverage state; track coverage in [component parity](docs/design/app/component-parity.md).
...
4. **Drive Whole Runs With `cmd/poemdrive`, Not Ad-Hoc Scripts**: For any repeatable interaction — a perf measurement, a rendering check, an end-to-end capture on Windows or Android — write or reuse a scene file in `scenes/` and run `go run ./cmd/poemdrive scenes/<name>.json`. The scene's `launch` block self-provisions the target (boots an AVD, builds/installs the APK, sets the inspection property, forwards the port, or starts and foregrounds the Windows build), so one command works from a cold machine; `-teardown` stops only what that run started. Do **not** hand-roll `adb install`/`am start`/`curl`/PowerShell launch-and-drive sequences — that is the fragile, token-heavy pattern the scenario runner exists to replace, and a scene is a reusable fixture where a script is not. Canonical reference: [Scenario replay](docs/design/automation/perf-profiling.md).
5. **Window Control Must Stay Narrow**: `prepare-window` and related native debug controls should remain dev/test oriented and limited to safe operations like restore, clamp, and foreground requests. Avoid turning the automation layer into a broad arbitrary window-management API.
6. **Document New Endpoints Immediately**: If automation endpoints, headers, state fields, or inspection semantics change, update the relevant [`docs/design/automation/`](docs/design/automation/TDD.md) document, `README.md`, and any relevant tests in the same change.
7. **Don't Leak Internals Over HTTP**: Automation endpoint handlers should return appropriate status codes and error bodies, never raw panic output, stack traces, or internal error details.
...
- `docs/design/architecture/TDD.md`: runtime architecture, UI authoring, and component/layout design (replaces the removed `ARCHITECTURE.md` and `GUIDE.md`)
- `docs/design/automation/TDD.md`: HTTP automation API, capture semantics, and inspection workflow (replaces the removed `docs/AUTOMATION.md`)
- `docs/reference/public-api.md`, `docs/reference/protocol.md`: public contracts and the shared presenter protocol
