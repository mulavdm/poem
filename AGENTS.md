...
6. **Document New Endpoints Immediately**: If automation endpoints, headers, state fields, or inspection semantics change, update the relevant `okf/concepts/automation/` concept, `README.md`, and any relevant tests in the same change.
...
- The OKF specification and bundle-authoring methodology live in the single workspace-root `OpenKnowledgeFormat/` reference checkout, not inside this repository. Never modify it for POEM work — inspect it for the spec when changing OKF structure, frontmatter conventions, or link policy, rather than guessing.
...
- `okf/log.md` is a chronological documentation ledger, not a release changelog — add an entry under today's date for every bundle change. The order is **code → concepts → ledger**: update the concepts the change actually altered, then record *which concepts moved* using the existing `**Modified Concepts:**` / `**Added Concept:**` markers. An entry that narrates a code change but names no concepts means that middle step was skipped. A behaviour no concept documents is a gap to fill, not a reason to skip the update.
- Verify the bundle with `pwsh scripts/check-okf.ps1` before committing. It is a thin wrapper holding no rules of its own: it runs the shared `okfcheck` from the workspace-root `OpenKnowledgeFormat/tools/okfcheck` (falling back to a repo-root-adjacent checkout, e.g. in CI), and this bundle's stricter policy (the five required frontmatter fields, bundle-absolute links, strict mode) lives in `okf/.okfcheck`. If neither location has the reference checkout, the script fails with clone instructions rather than silently passing; CI clones one next to the repo root before running it. Do not reimplement these rules in this repository; MAPPS runs the identical wrapper, and two copies of one rule set drift.
