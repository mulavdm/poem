...
- Verify the bundle with `pwsh scripts/check-okf.ps1` before committing. It is a thin wrapper holding no rules of its own: it runs the shared `okfcheck` from `OpenKnowledgeFormat/tools/okfcheck`, and this bundle's stricter policy (the five required frontmatter fields, bundle-absolute links, strict mode) lives in `okf/.okfcheck`. Because that reference folder is Git-ignored, a clean checkout lacks it — the script fails with clone instructions rather than silently passing, and CI clones the folder before running it. Do not reimplement these rules in this repository; MAPPS runs the identical wrapper, and two copies of one rule set drift.
