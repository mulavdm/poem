...
1.  **Idiomatic Modularity**: Favor small, focused packages. Keep OS-specific syscalls in `internal/win32`, modular rendering logic inside `pkg/render` (with `types`, `components`, `layout`, and `backend` subpackages), and application entry in `cmd/engine`.
...
4.  **Standalone Library Design**: Treat PolyEngine as a consumable package. All public interactions must happen through the root `pkg/render` package's type-aliased public interfaces and event-loop runner. Implementation details from `internal/win32` or specific backends must never leak into the user's application code.
...
- **Run Benchmarks**: `go test -v -bench="." go_native_gpu_gui/pkg/render/backend`
