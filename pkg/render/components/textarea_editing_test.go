...
func TestTextAreaUnicodeSelectionCutAndMultilinePaste(t *testing.T) {
	...
	area.Text = "Go 日本語\nsecond"
	area.CursorIndex = len([]rune(area.Text))
	...
	if !area.OnKey('X', 0, state) || area.Text != "Go \nsecond" {
		t.Fatalf("cut text %q", area.Text)
	}
	clipboard.text = "first\r\n世界"
	if !area.OnKey('V', 0, state) || area.Text != "Go first\n世界\nsecond" {
		t.Fatalf("pasted text %q", area.Text)
	}
}
...
func TestTextAreaSelectionReplacementAndReadOnlyCopy(t *testing.T) {
	...
	state := editingTestState(clipboard)
	area := NewTextArea("input", "")
	area.Text = "replace me"
	area.SetSelection(0, len([]rune(area.Text)), state)
	if !area.OnKey(0, '✓', state) || area.Text != "✓" {
		t.Fatalf("replacement text %q", area.Text)
	}
	area.Text, area.ReadOnly = "locked", true
	...
	if area.OnKey('V', 0, state) || area.Text != "locked" {
		t.Fatalf("read-only text mutated to %q", area.Text)
	}
}
...
func TestTextAreaRejectsOversizedPaste(t *testing.T) {
	...
	state.KeysPressed[0x11] = true
	area := NewTextArea("input", "")
	if area.OnKey('V', 0, state) || area.Text != "" {
...
func TestTextAreaCompositionPreviewCommitAndCancel(t *testing.T) {
	...
	if !area.EndComposition("日本", state) || area.Text != "one 日本" {
		t.Fatalf("committed %q", area.Text)
	}
	area.SetSelection(6, 6, state)
	if !area.StartComposition(state) || !area.UpdateComposition("語", state) || !area.EndComposition("", state) || area.Text != "one 日本" {
		t.Fatalf("cancel mutated %q", area.Text)
	}
}
