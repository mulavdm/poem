...
The result is packed into a `MapSceneDelta` and shipped over the [presenter protocol](/concepts/protocol.md); the native runtime (`pkg/app/desktop/map_runtime.go`) reconciles scenes per viewport, generation-fenced, and honours the `MapCachePolicy`.
