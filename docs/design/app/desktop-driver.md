...
`Open`/`OnOpenChange` unset — see [Architecture Overview](../app/TDD.md) for why),
...
[Architecture Overview](../app/TDD.md) for the full `Node` set and the policy on
adding more.
...
[Architecture Overview](../app/TDD.md) for why that specific placement matters
