...
[Overview](../app/TDD.md) — "There is no partial-coverage state").
...
| `MapViewportNode` | `ImageViewport` (retained map scene) | `poem-map-viewport` canvas + WebGL2/WASM | ✅ GPU vector map, camera + feature messages, raster fallback | See [GPU Vector Map Rendering](../architecture/map-rendering.md). |
...
| `OverlayNode`    | `Overlay`           | `position:relative` box + absolutely-positioned layers | ✅ base fills; layers anchor + hit-test above base | Non-modal z-stack for canvas controls; see [Desktop Driver](../app/desktop-driver.md). |
...
the [log](../../archive/okf-bundle-log.md) and, being a shared-engine change, in POEM's own log too.
...
[log](../../archive/okf-bundle-log.md), not a silent edit here.
